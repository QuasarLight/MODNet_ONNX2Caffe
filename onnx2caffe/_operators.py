...
def _convert_Mul(node,graph,err):
    ...
    if input_name_list[0] == input_name_list[1]:
        ...
        input_A_h_shape = graph.shape_dict[node.inputs[0]][2]
        input_A_w_shape = graph.shape_dict[node.inputs[0]][3]
        input_name_A = input_name_list[0]
        input_name_B = input_name_list[1]
        layer_tile_h = myf("Tile",node_name+'_tile_h',[input_name_B],[input_name_B+'_tile_h'],tile_param = dict(axis=2,tiles=input_A_h_shape))
        layer_tile_w = myf("Tile",node_name+'_tile_w',[input_name_B+'_tile_h'],[input_name_B+'_tile_w'],tile_param = dict(axis=3,tiles=input_A_w_shape))
        layer = myf("Eltwise",node_name,[input_name_A,input_name_B+'_tile_w'],[output_name],operation=P.Eltwise.PROD)
        
        graph.channel_dims[output_name] = graph.channel_dims[input_name_list[0]]
        return layer_tile_h,layer_tile_w,layer
...
def _convert_conv_slice(node, graph, err):
    ...
    output_name = str(node.outputs[0])
    node_name = node.name
    axes = node.input_tensors.get(node.inputs[3])
    #print(axes)
    ...
    starts = node.input_tensors.get(node.inputs[1])
    ends = node.input_tensors.get(node.inputs[2])
    axes = node.input_tensors.get(node.inputs[3])
    start = starts[0]
    end = ends[0]
    if end > 1000:
...
def _convert_clip(node,graph,err):
    ...
    layer = myf("Clip",name,[input_name],[output_name],clip_param = dict(min=0,max=6))
...
def _convert_resize(node,graph,err):
    ...
    if scales[2] > 1 and scales[3] > 1:
        ...
        kernel_h = int(1/scales[2])
        kernel_w = int(1/scales[3])
        layer = myf("Pooling",node_name,[input_name],[output_name],pooling_param = dict(pool = P.Pooling.AVE,
                                                                                    kernel_h = kernel_h,
                                                                                    kernel_w = kernel_w,
                                                                                    stride_h = kernel_h,
...
def _convert_sub(node,graph,err):
    input_name_A = str(node.inputs[0])
    input_name_B = str(node.inputs[1])
    output_name = str(node.outputs[0])
    name = str(node.name)
    ...
    layer_tile_h = myf("Tile",name+'_tile_h',[input_name_B],[input_name_B+'_tile_h'],tile_param = dict(axis=2,tiles=input_A_h_shape))
    layer_tile_w = myf("Tile",name+'_tile_w',[input_name_B+'_tile_h'],[input_name_B+'_tile_w'],tile_param = dict(axis=3,tiles=input_A_w_shape))
    layer = myf("Eltwise",name,[input_name_A,input_name_B+'_tile_w'],[output_name],operation=P.Eltwise.SUM,coeff=[1,-1])

    graph.channel_dims[output_name] = graph.channel_dims[input_name_A]
...
def _convert_div(node,graph,err):
    ...
    layer_power = myf("Power",name+'_power',[input_name_B],[input_name_B],in_place=True,power=-1)
    
    # layer = myf("Scale",name,[input_name_A,input_name_B],[output_name],bias_term=False)
    
    input_A_h_shape = graph.shape_dict[node.inputs[0]][2]
    input_A_w_shape = graph.shape_dict[node.inputs[0]][3]
    layer_tile_h = myf("Tile",name+'_tile_h',[input_name_B],[input_name_B+'_tile_h'],tile_param = dict(axis=2,tiles=input_A_h_shape))
    layer_tile_w = myf("Tile",name+'_tile_w',[input_name_B+'_tile_h'],[input_name_B+'_tile_w'],tile_param = dict(axis=3,tiles=input_A_w_shape))
    layer = myf("Eltwise",name,[input_name_A,input_name_B+'_tile_w'],[output_name],operation=P.Eltwise.PROD)

    graph.channel_dims[output_name] = graph.channel_dims[input_name_A]

    return layer_power,layer_tile_h,layer_tile_w,layer
